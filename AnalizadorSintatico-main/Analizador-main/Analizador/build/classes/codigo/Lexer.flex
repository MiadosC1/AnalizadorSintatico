...
(for|while|do|foreach|repeat|until)  
                               { lexeme=yytext(); return Ciclos; }

(switch|case|default|when)  
                               { lexeme=yytext(); return Condiciones; }

(print|scanf|cin|cout|input|output)  
...
(async|thread|synchronized)  
                               { lexeme=yytext(); return Concurrencia; }

(operator|override|explicit|virtual|inline|abstract)  
...
(union|tuple|delegate|event)  
                               { lexeme=yytext(); return EstructurasAdicionales; }

(stream|yield|pipe|fork|join)  
...
(typedef)                      { lexeme=yytext(); return Alias; }
...
(requires)                     { lexeme=yytext(); return Conceptos; }
...
([_$@])                        { lexeme=yytext(); return VariablesEspeciales; }

('[a-zA-Z]'|0x[0-9A-Fa-f]+)    { lexeme=yytext(); return LiteralesEspeciales; }
...
(export)                       { lexeme=yytext(); return Modularizacion; }
...
("0x"([0-9]|[a-fA-F])+)        { lexeme = yytext(); return NumeroHexadecimal; }

(\'({L}|{D})\')                { lexeme = yytext(); return Caracter; }

(\"({L}|{D})*\")               { lexeme = yytext(); return Cadena; }

("extern")                     { lexeme = yytext(); return EnlaceExterno; }
